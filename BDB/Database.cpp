...
static void CDatabaseTrace(const DbEnv* /*dbenv*/, const char* msg)
{
	LG_TRACE("%s", msg);
}

CDatabase::CDatabase()
    //: checkpoint(this, &CDatabase::Checkpoint),
    //  checkpointTimeout(&onCheckpointTimeout),
    //  onCheckpointTimeout(this, &CDatabase::OnCheckpointTimeout)
{
}
...
CTable* CDatabase::GetTable(const char* name)
{
...
void CDatabase::OnCheckpointTimeout()
{
...
static void WarmCDatabaseCache(char* dbPath, unsigned cacheSize)
{
	char filepath[4096];
...
void WarmCache(char* dbPath, unsigned cacheSize)
{
	...
	WarmLogCache(dbPath);
	WarmCDatabaseCache(dbPath, cacheSize);
