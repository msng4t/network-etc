 ...
 *
 *
 * Version: $Id: mem_cache.cpp 1008 2012-07-26 07:10:15Z fengmao $
 *
 * Authors:
...
namespace tair {
  ...
  void mem_cache::calc_slab_balance_info(std::map<int, int> &adjust_info)
  {
    ...
    LOG_WARN("should_total:%f,crrnt_no:%f,evict_ratio:%f",
    ...
    for(map<int, int>::iterator it = adjust_info.begin();
        it != adjust_info.end(); ++it) {
      LOG_WARN(" (%d) : %d", it->first, it->second);
    }
  ...
  {
    LOG_WARN("total slab : %u", slab_managers.size());
 ...
 *-----------------------------------------------------------------------------*/
  ...
  mdb_item *mem_cache::slab_manager::alloc_new_item(int area)
  {
    ...
    LOG_DEBUG( "from page id:%u,now free:%d,free-head:%lu,id will be use:%lu", info->id, info->free_nr,info->free_head,item_id);
    ...
    else
    {
      ...
      {
        LOG_ERROR("the page id of free_head [%d] != info->id [%d]",PAGE_ID(info->free_head),info->id);
      }
    }
    return item;
  }
 ...
 */
  ...
  void mem_cache::slab_manager::free_item(mdb_item * item)
  {
    ...
    LOG_DEBUG( "after free:page id:%d,%p,info->free_nr:%d,info->free_head:%lu,SLAB_ID(id):%d,info->free_head->next : %lu",
 ...
 */
  ...
  mdb_item *mem_cache::slab_manager::evict_any(int &type)
  {
    ...
    LOG_TRACE("evict any,area:%d",type);
    uint32_t crrnt_time = time(NULL);
    mdb_item *item = 0;
    int times = 0;
    int area = type;
