 ...
 *
 *
 * Version: $Id: mdb_manager.cpp 1063 2012-08-27 03:30:42Z zongdai $
 *
 * Authors:
...
//        TAIR_STAT.stat_evict(ITEM_AREA(it));
  ...
  int mdb_manager::clear(int area)
  {
    ...
    if (area == -1){
      ...
      assert(area >= 0 && area < TAIR_MAX_AREA_COUNT);
      boost::mutex::scoped_lock guard(mem_locker);
      LOG_DEBUG("clear all the items in the area =%u",area);
                                   ...
                                   vector<item_data_info *> &list)
  {
    ...
    while(item_head != 0) {
      ...
      uint32_t server_hash =
        mur_mur_hash(ITEM_KEY(it) + 2, it->key_len - 2);
                          ...
                          bool version_care, int expired)
  {
    ...
    {
      LOG_DEBUG("key.area[%d] != KEY_AREA(key.get_data())[%d]",key.area,KEY_AREA(key.get_data()));
    ...
    if(it != 0) {                //exists
      ...
      if(is_item_expired(it, crrnt_time)) {
        ...
        LOG_WARN("it->version(%hu) != version(%hu)", it->version,
                  key.get_version());
        ...
        LOG_DEBUG("%s:already exists,remove it", __FUNCTION__);
        __remove(it);
        it = 0;
      }
    }
...
//        TAIR_STAT.stat_evict(ITEM_AREA(it));
  ...
  int mdb_manager::do_get(data_entry & key, data_entry & data, bool with_stat)
  {
    ...
    LOG_DEBUG("start get: area:%d,key size:%u", key.area,
              key.get_size());
...
//  }
  ...
  void mdb_manager::__remove(mdb_item * it)
  {
    ...
    it->version = 0;
    it->update_time = 0;
    LOG_DEBUG("after free item [%p],id [%lu],h_next [%lu] next [%lu],prev[%lu],key_len[%d],data_len[%lu], version[%d]",it,it->item_id,it->h_next,it->next,it->prev,it->key_len,it->data_len,it->version);
  }
  ...
  {
    LOG_WARN("start remove expired mdb_item ...");
    int64_t del_count = 0;
    ...
    LOG_WARN("end remove expired item [%ld] [%ld]",del_count,release_space);
  ...
  void mdb_manager::balance_slab()
  {
   ...
   LOG_TRACE("slabinfo.size():%d", slab_info.size());

    for(map<int, int>::iterator it = slab_info.begin();
        it != slab_info.end(); ++it) {
      if(it->second < 0) {
       LOG_TRACE("from [%d] free  [%d] page(s)", it->first,
            it->second);
        for(int i = 0; i < -it->second; ++i) {
          {
            boost::mutex::scoped_lock guard(mem_locker);
            if(cache->free_page(it->first) != 0) {
              LOG_WARN( "free page failed");
              break;
            }
          }
        }
