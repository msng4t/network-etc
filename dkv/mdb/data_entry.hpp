 ...
 *
 * Version: $Id: data_entry.hpp 938 2012-06-15 08:01:55Z yexiang $
 *
 * Authors:
...
#define MAX_SHOW_KEY_LEN 128
            ...
            {
                uint8_t liHas_merged = has_merged;
                output->Write(liHas_merged);
                output->Write(area);
                output->Write(server_flag);
                data_meta.encode(output);

                uint32_t msize = (size | (prefix_size << PREFIX_KEY_OFFSET));
                output->Write(msize);
                if (get_size() > 0) {
                    output->Write(get_data(), get_size());
                }
            }
            bool decode(mina::CMemblock *input)
            {
                free_data();
                uint8_t temp_merged;
                uint32_t _area;
    ...
    struct data_entry_hash {
        size_t operator()(const data_entry *a) const
        {
            ...
            return mur_mur_hash(a->get_data(), a->get_size());
        }
    };
