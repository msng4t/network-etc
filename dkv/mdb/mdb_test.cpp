 ...
 *
 *
 * Version: $Id: mdb_test.cpp 470 2011-12-28 05:26:28Z zongdai $
 *
 * Authors:
...
put_thread(void *arg)
{
  ...
  for(int i = 0; i < ta->max_loop; i += report_per_times) {
    ...
    LOG_WARN("[%d]in %d seconds,success:%d,failed:%d,total data : %ld bytes",
...
remove_thread(void *arg)
{
  ...
  for(int i = 0; i < ta->max_loop; i += report_per_times) {
    ...
    LOG_WARN("[%d]in %d seconds,success:%d,failed:%d", ta->index,
...
main(int argc, char *argv[])
{
  ...
  LOG_WARN("thread_count : %d,loop times per thread : %d,min_data_size : %d,max_data_size : %d,report_interval : %d,mdb size : %dM",
